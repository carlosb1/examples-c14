...
string bucketName = "kerberostest";
...
class S3Uploader
{
    ...
    static void upload()
    {
        ...
        while(true)
        {
            ...
            { 
                string file = *it;
                bool hasBeenUploaded = s3.putObject(file, bucketName);
...
{
       S3 s3;
       std::string output = s3.getObjects("prodcast-stress");
       std::cout << output << std::endl;

       std::string example_directory{"/home/carlosb/Desktop/test_s3"};
       vector<string> storage;
       Helper::getFilesInDirectory(storage, example_directory); // get all symbol links of directory
	//vector<string>::iterator it = storage.begin();
       for (auto string_to_upload: storage) {
	       std::cout << string_to_upload << "\n";
	       s3.putObject(string_to_upload, "prodcast-stress");
       }

//       output = s3.getObject("/havas8b.jpg", "prodcast-stress");
//       std::cout << output << std::endl;
