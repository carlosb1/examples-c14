...
string S3::getObjects(const string & bucket)
{
   vector<string> headers;
    ...
    request += date + "\n";
    request += "/" + bucket + "/";
    std::string auth="Authorization: " + authorize(request);
    
    // Url
    string url = "https://s3.eu-west-1.amazonaws.com/"; 
    
    vector<string> new_headers;
    new_headers.push_back("User-Agent: aws-sdk-nodejs/2.339.0 darwin/v10.9.0 callback");
    new_headers.push_back("Content-Length: 0");
    new_headers.push_back("Host: prodcast-stress.s3.eu-west-1.amazonaws.com");
    new_headers.push_back("Date: "+date);
    new_headers.push_back(auth);
    new_headers.push_back("Connection: keep-alive");

    // Execute get operation
    return get(url, new_headers);
}

string S3::getObject(const string & file, const string & bucket)
{
    // File name
    ...
    request += date + "\n";
    // request += "/" + file + "/";
    request += "/"+bucket+"/" + fileName;
    std::string auth="Authorization: " + authorize(request);
    
    // Url
    // string url = "https://s3.eu-west-1.amazonaws.com/" + fileName; 
...
//content type
//hash control 
bool S3::putObject(const string & file, const string & bucket)
{
    vector<string> headers;
...
bool S3::put(const string & url, const vector<string> & headers, const vector<string> & body)
{
    ...
    //TODO change to work with the buffer file
    // Intialize body
    string file = body[0];
